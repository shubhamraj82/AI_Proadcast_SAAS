...
export const updateProjectStatus = mutation({
  ...
  handler: async (ctx, args) => {
    const updates: Partial<{ status: string; updatedAt: number; completedAt?: number }> = {
      status: args.status,
      updatedAt: Date.now(),
    };
...
export const updateJobStatus = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const updates: Partial<typeof project> = {
      jobStatus: {
        ...project.jobStatus,
        ...(args.transcription !== undefined && { transcription: args.transcription }),
        ...(args.contentGeneration !== undefined && { contentGeneration: args.contentGeneration }),
      },
      updatedAt: Date.now(),
    };
...
export const listUserProjects = query({
  ...
  handler: async (ctx, args) => {
    ...
    // The .withIndex("by_user", ...) line has a type error due to index definitions.
    // Instead, filter by userId directly.
    const query = ctx.db
      .query("projects")
...
export const getUserProjectCount = query({
  ...
  handler: async (ctx, args) => {
    ...
    const projects = await ctx.db
      .query("projects")
      // If you don't have an index or the .withIndex signature doesn't match,
      // just filter without the index.
